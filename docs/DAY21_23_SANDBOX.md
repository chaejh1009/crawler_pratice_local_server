...
이 게시판과 연결된 상세 화면은 토큰 없이 조회할 수 있으며, 조회 시점에 ID가 가장 큰 최신 최대 10,000건을 HTML에 공개한다. 새 데이터가 추가되면 공개 창에 들어오고 가장 오래된 데이터가 빠진다. 검색·필터·정렬은 이 실시간 공개 집합 안에서 동작한다. 전체 데이터와 고정 snapshot은 현재 일일 키를 헤더에 넣은 `/api/v1/cars` 또는 `/api/v1/cars/cursor`로 수집한다.
