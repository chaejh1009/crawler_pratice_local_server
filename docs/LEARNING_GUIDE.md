...
`/cars`와 연결된 상세 화면은 인증 없이 열리며, 조회 시점에 ID가 가장 큰 최신 최대 10,000건을 HTML로 공개합니다. 새 데이터가 들어오면 공개 창에 추가되고 가장 오래된 데이터가 빠집니다. 필터·정렬은 이 실시간 집합 안에서 동작합니다. 전체 데이터의 고정 snapshot 순회는 현재 일일 키를 헤더에 넣은 `/api/v1/cars/cursor`를 사용합니다.

기본 배포는 4분마다 28건, 하루 약 10,080건의 새 합성 차량을 추가합니다. 그래서 장시간 API 수집에서는 page 번호만 늘리지 말고 첫 커서 응답의 `until_id`와 `dataset_epoch`를 고정하여 유한한 snapshot을 만든 뒤, 이후 변화는 `/api/v1/changes`로 이어 받습니다.
...
`/api/v1/*`의 데이터 API에는 키가 필요합니다. `/api/v1/public-key`는 오늘의 키를 받기 위한 공개 예외이며, `/healthz`, 실시간 최신 최대 10,000건 범위의 HTML 게시판, 정적 파일도 공개입니다.
