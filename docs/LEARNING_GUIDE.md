...
`/cars`와 연결된 상세 화면은 인증 없이 열리지만, 데이터셋에서 고정한 최초 최대 10,000건만 HTML로 공개합니다. 필터·정렬을 바꾸어도 이 집합 밖의 데이터는 나오지 않습니다. 전체 데이터 순회는 현재 일일 키를 헤더에 넣은 `/api/v1/cars/cursor`를 사용합니다.
...
`/api/v1/*`의 데이터 API에는 키가 필요합니다. `/api/v1/public-key`는 오늘의 키를 받기 위한 공개 예외이며, `/healthz`, 최대 10,000건 범위의 HTML 게시판, 정적 파일도 공개입니다.
