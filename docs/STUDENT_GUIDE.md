...
`robots.txt`는 최대 10,000건의 공개 HTML 게시판과 공개 키 경로를 안내하고 그 외 `/api/`를 익명 순회하지 않도록 제한합니다. JSON 데이터 API에는 공개된 오늘의 키를 헤더에 넣습니다. HTML 수집 요청 사이에는 최소 1초를 기다리세요. 이 서버의 수집 허가는 다른 웹사이트에 적용되지 않습니다.
...
| --- | --- | --- |
| `/` | 공개 | 데이터셋 현황과 실습 메뉴 |
| `/cars` | 공개 | 최초 최대 10,000건 안에서 차량 검색·필터·정렬·페이지 이동 |
| `/cars/:id` | 공개 | 위 공개 집합에 속한 차량 한 건의 상세 정보 |
...
`/cars`는 토큰 없이 접근하는 게시판 형식이며 기본 주소 `/cars?page=1&page_size=20`에서 한 페이지에 20건을 보여 줍니다. 이 HTML 경로의 수집 대상은 데이터셋에서 고정한 최초 최대 10,000건입니다. 검색어, 브랜드, 연료, 판매 상태, 차량 소재지, 최소 연식, 최대 주행거리, 최소·최대 가격, 정렬 방식, 표시 개수를 바꾸어도 그 집합 밖의 데이터는 나오지 않습니다. 표시 개수는 20, 40, 60, 100개 중에서 고를 수 있습니다.
...
서버는 기본적으로 약 4분마다 28건, 하루 약 10,080건의 새 합성 차량을 추가합니다. 공개 HTML은 최초 최대 10,000건 범위에 머물지만 인증 API와 변경 로그는 계속 증가할 수 있습니다. 전체 순회 도중 데이터가 늘어나는 문제는 커서 첫 응답의 `until_id`와 `dataset_epoch`를 끝까지 고정해 해결합니다.
